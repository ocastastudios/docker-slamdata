...
RUN apt-get update && apt-get -y install npm && npm install bower -g && \
    ...
    wget -q -O slamdata.tar.gz https://github.com/slamdata/slamdata/archive/v3.0.3.tar.gz && \
    ...
    wget -O quasar.jar -q https://github.com/quasar-analytics/quasar/releases/download/v10.1.13-quasar-web/quasar-web_2.11-10.1.13-one-jar.jar && \
    chmod a+x /run.sh
